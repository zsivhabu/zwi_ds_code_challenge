...
export default defineComponent({
  ...
  setup: function (props) {
    ...
    const selectOptionsFromStore = useSelectLists().causeCodeGroupList;
    const selectOptionsFromServer = new SelectListService().getSelectOptions('cause_code_group',true);
...
<template>
  <generic-server-select-list
      ...
      store-method-name="causeCodeGroupList"/>
</template>